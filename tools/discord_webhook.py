...
class Tools:
    ...
    class Valves(BaseModel):
        ...
        def validate_webhook_url(cls, v):
            ...
            if not v.startswith("https://discord.com/api/webhooks/"):
                 logging.warning("Webhook URL does not look like a standard Discord webhook URL.")
            return v
    ...
    async def _ensure_session(self):
        ...
        if self.session is None or self.session.closed:
             if self._session_externally_managed:
                 ...
                 raise RuntimeError("External aiohttp session is closed or was not provided.")
             else:
    ...
    async def send_message(self, message_content: str) -> bool:
        ...
        try:
            ...
            if len(message_content) > 2000:
                logging.warning("Message content exceeds Discord limit (2000 characters). Truncating.")
            ...
            logging.exception(f"An unexpected error occurred while sending the message: {e}")
            return False
    ...
    async def close(self):
        ...
        """
        if not self._session_externally_managed and self.session and not self.session.closed:
...
async def main():
    ...
    if not os.environ.get("DISCORD_WEBHOOK_URL"):
        ...
        print("Please set it to run the example: export DISCORD_WEBHOOK_URL='your_webhook_url_here'")
        return
    ...
    async with aiohttp.ClientSession() as external_session:
        try:
            ...
            success1 = await discord_tool_external.send_message("First message with external session.")
            print(f"Message 1 sent successfully: {success1}")

            # Reuse the same tool instance (and the external session)
            success2 = await discord_tool_external.send_message("Second message with the same external session.")
